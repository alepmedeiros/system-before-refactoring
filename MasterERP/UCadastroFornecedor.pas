...
uses
  ...
  ExtCtrls, ComCtrls, Grids, DBGrids, JvExMask, JvToolEdit, JvDBControls, Mask,
  Menus;

type
  TCadastroFornecedorForm = class(TCadastroPadraoForm)
...
var
  CadastroFornecedorForm: TCadastroFornecedorForm;

implementation
uses Base, UFuncoes, UPesquisaMunicipio, UVerificaPessoa;
{$R *.dfm}

procedure TCadastroFornecedorForm.VerificaCampos;
begin
...
procedure TCadastroFornecedorForm.CarregaHint;
begin
  CBTipo.Hint := 'Tipo de Fornecedor (Pessoa Física/Pessoa Jurídica).';

  DBEditCodigo.Hint := 'Este é o Código de identificação do Fornecedor.' +
    ' Ele é gerado automaticamente pelo Sistema';

  case CBTipo.ItemIndex of
    0: begin
      DBEditNome.Hint := 'Informe aqui o Nome do Fornecedor.' +
        ' Verifique antes se o Nome informado já existe pressionando a tecla F2.';
      DBEditApelido.Hint := 'Informe aqui o Apelido do Fornecedor.';
      DBEditDocumento.Hint := 'Informe aqui o CPF do titular do Fornecedor.' +
        ' Digite apenas Números. O Sistema verificará sua validade' +
        ' pressionando a tecla Enter.';
      DBEditIE.Hint := 'Informe aqui o Número de Identidade.' +
        ' Digite apenas Números. O Sistema verificará sua validade' +
        ' pressionando a tecla Enter.';
      DBDataNascimento.Hint := 'Infome aqui a Data de Nascimento do titular do Fornecedor.';
    end;
    1: begin
      DBEditNome.Hint := 'Informe aqui a Razão Social do Fornecedor.' +
        ' Verifique antes se a Razão Social informada já existe pressionando a tecla F2.';
      DBEditApelido.Hint := 'Informe aqui o Nome Fantasia do Fornecedor.';
      DBEditDocumento.Hint := 'Informe aqui o CNPJ do Fornecedor.' +
        ' Digite apenas Números. O Sistema verificará sua validade' +
        ' pressionando a tecla Enter.';
      ...
      DBEditIM.Hint := 'Informe aqui a Inscrição Municipal.' +
        ' Digite apenas Números. O Sistema verificará sua validade' +
        ' pressionando a tecla Enter.';
      DBDataNascimento.Hint := 'Infome aqui a Data de Abertura do Fornecedor.';
    end;
  end;
  ...
  DBNEndereco.Hint := 'Alterar Endereços do Fornecedor. É possível Inserir vários endereços' +
    ' para cada Cliente.';
...
procedure TCadastroFornecedorForm.CBPessoaContatoTipoSelect(Sender: TObject);
begin
...
procedure TCadastroFornecedorForm.CBTipoSelect(Sender: TObject);
begin
  case CBTipo.ItemIndex of
    0: begin
      LBNome.Caption := 'Nome:';
...
procedure TCadastroFornecedorForm.CDSCadastroAfterInsert(DataSet: TDataSet);
begin
  try
    with BancoDados.qryGeraID do
      begin
        ...
        Close;
        SQL.Clear;
        SQL.Add('select gen_id( GEN_FORNECEDOR_ID, 1 ) from RDB$DATABASE');
        Open;
        CDSCadastroFORNECEDOR_ID.Value := BancoDados.qryGeraID.Fields[0].Value;
        CDSCadastroTIPO.Value := 'F';
        GeraTrace('FORNECEDOR','ID Gerado');
...
procedure TCadastroFornecedorForm.CDSCadastroBeforePost(DataSet: TDataSet);
begin
...
procedure TCadastroFornecedorForm.DBEditDocumentoExit(Sender: TObject);
var
  Temp : ShortString;
begin
...
procedure TCadastroFornecedorForm.DBGrid1KeyPress(Sender: TObject;
  var Key: Char);
begin
  if (key = #13) then
    begin
...
procedure TCadastroFornecedorForm.DBNEnderecoClick(Sender: TObject;
  Button: TNavigateBtn);
begin
...
procedure TCadastroFornecedorForm.ExcluirContato1Click(Sender: TObject);
begin
...
procedure TCadastroFornecedorForm.FormClose(Sender: TObject;
...
procedure TCadastroFornecedorForm.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if (key = #13) then
    begin
      key := #0;
...
procedure TCadastroFornecedorForm.FormShow(Sender: TObject);
var
...
procedure TCadastroFornecedorForm.PCPrincipalChange(Sender: TObject);
begin
  if (PCPrincipal.TabIndex = 1) then
    CBPessoaContatoTipoSelect(Sender);
end;

procedure TCadastroFornecedorForm.RemoveAcento(Sender: TObject);
var
  Temp : ShortString;
begin
...
procedure TCadastroFornecedorForm.AVerificarNomeExecute(Sender: TObject);
begin
  try
...
procedure TCadastroFornecedorForm.BTCancelarClick(Sender: TObject);
begin
  BancoDados.CDSPessoa.Cancel;
...
procedure TCadastroFornecedorForm.BTMunicipioClick(Sender: TObject);
begin
  try
...
procedure TCadastroFornecedorForm.BTSalvarClick(Sender: TObject);
begin
  VerificaCampos;
