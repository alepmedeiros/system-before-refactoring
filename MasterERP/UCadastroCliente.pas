...
uses
  ...
  ExtCtrls, ComCtrls, Grids, DBGrids, JvSpin, JvDBSpinEdit, JvExMask,
  JvToolEdit, JvDBControls, Mask, Menus;
...
procedure TCadastroClienteForm.VerificaCampos;
begin
...
procedure TCadastroClienteForm.CarregaHint;
begin
  CBTipo.Hint := 'Tipo de Cliente (Pessoa Física/Pessoa Jurídica).';

  DBEditCodigo.Hint := 'Este é o Código de identificação do Cliente.' +
    ' Ele é gerado automaticamente pelo Sistema';

  case CBTipo.ItemIndex of
    0: begin
      DBEditNome.Hint := 'Informe aqui o Nome do Cliente.' +
        ' Verifique antes se o Nome informado já existe pressionando a tecla F2.';
      DBEditApelido.Hint := 'Informe aqui o Apelido do Cliente.';
      DBEditDocumento.Hint := 'Informe aqui o CPF do titular do Cliente.' +
        ' Digite apenas Números. O Sistema verificará sua validade' +
        ' pressionando a tecla Enter.';
      DBEditIE.Hint := 'Informe aqui o Número de Identidade.' +
        ' Digite apenas Números. O Sistema verificará sua validade' +
        ' pressionando a tecla Enter.';
      DBDataNascimento.Hint := 'Infome aqui a Data de Nascimento do titular do Cliente.';
    end;
    1: begin
      DBEditNome.Hint := 'Informe aqui a Razão Social do Cliente.' +
        ' Verifique antes se a Razão Social informada já existe pressionando a tecla F2.';
      DBEditApelido.Hint := 'Informe aqui o Nome Fantasia do Cliente.';
      DBEditDocumento.Hint := 'Informe aqui o CNPJ do Cliente.' +
        ' Digite apenas Números. O Sistema verificará sua validade' +
        ' pressionando a tecla Enter.';
      ...
      DBEditIM.Hint := 'Informe aqui a Inscrição Municipal.' +
        ' Digite apenas Números. O Sistema verificará sua validade' +
        ' pressionando a tecla Enter.';
      DBDataNascimento.Hint := 'Infome aqui a Data de Abertura do Cliente.';
    end;
  end;
  ...
  DBNEndereco.Hint := 'Alterar Endereços do Cliente. É possível Inserir vários endereços' +
    ' para cada Cliente.';
...
procedure TCadastroClienteForm.RemoveAcento(Sender: TObject);
var
  Temp : ShortString;
begin
...
procedure TCadastroClienteForm.AVerificarNomeExecute(Sender: TObject);
begin
  try
...
procedure TCadastroClienteForm.BTCancelarClick(Sender: TObject);
begin
  BancoDados.CDSPessoa.Cancel;
...
procedure TCadastroClienteForm.BTMunicipioClick(Sender: TObject);
begin
  try
...
procedure TCadastroClienteForm.BTSalvarClick(Sender: TObject);
begin
  VerificaCampos;
...
procedure TCadastroClienteForm.CBPessoaContatoTipoSelect(Sender: TObject);
begin
...
procedure TCadastroClienteForm.CBTipoSelect(Sender: TObject);
begin
  case CBTipo.ItemIndex of
    0: begin
      LBNome.Caption := 'Nome:';
...
procedure TCadastroClienteForm.CDSCadastroAfterInsert(DataSet: TDataSet);
begin
  try
    with BancoDados.qryGeraID do
      begin
        ...
        Open;
        CDSCadastroCLIENTE_ID.Value := BancoDados.qryGeraID.Fields[0].Value;
        CDSCadastroTIPO.Value := 'F';
...
procedure TCadastroClienteForm.CDSCadastroBeforePost(DataSet: TDataSet);
begin
...
procedure TCadastroClienteForm.DBEditDocumentoExit(Sender: TObject);
var
  Temp : ShortString;
begin
...
procedure TCadastroClienteForm.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
  if (key = #13) then
    begin
...
procedure TCadastroClienteForm.DBNEnderecoClick(Sender: TObject;
  Button: TNavigateBtn);
begin
...
procedure TCadastroClienteForm.ExcluirContato1Click(Sender: TObject);
begin
...
procedure TCadastroClienteForm.FormClose(Sender: TObject;
...
procedure TCadastroClienteForm.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if (key = #13) then
    begin
      key := #0;
...
procedure TCadastroClienteForm.FormShow(Sender: TObject);
var
...
procedure TCadastroClienteForm.PCPrincipalChange(Sender: TObject);
begin
